...
describe DynamicImage::Model::Dimensions do
  ...
  describe "#crop_gravity" do
    ...
    context "when image is cropped" do
      let(:image) { Image.new(real_width: 320, real_height: 200, crop_width: 12, crop_height: 11, crop_start_x: 12, crop_start_y: 11) }
      it { is_expected.to eq(Vector2d.new(18, 16)) }
    end

    context "when gravity is set" do
      let(:image) { Image.new(crop_gravity_x: 200, crop_gravity_y: 100, real_width: 320, real_height: 200) }
  ...
  describe "#cropped?" do
    ...
    context "crop_size and real_size is set" do
      let(:image) { Image.new(crop_width: 200, crop_height: 100, real_width: 320, real_height: 200) }
      it { is_expected.to be true }
    end

    context "crop_size and real_size are the same" do
      let(:image) { Image.new(crop_width: 320, crop_height: 200, real_width: 320, real_height: 200) }
      it { is_expected.to be false }
    end
  end
  ...
  describe "#size" do
    ...
    context "when cropped" do
      let(:image) { Image.new(real_width: 320, real_height: 200, crop_width: 100, crop_height: 80) }
