...
module DynamicImage
  ...
  #
  # Everything needed to render an image responsively: the candidate widths,
  # the signed URLs behind them, and the fallback the <tt>img</tt> points at.
  #
  # Sizing is expressed as an optional ratio rather than a size.
  #
  # @example
  ...
  class Picture
    # The options {DynamicImage::Picture} consumes. Anything else is passed on to the router.
    OPTIONS = %i[ratio sizes breakpoints step fallback_width format uncropped].freeze

    # @!attribute [r] record
    #   @return [DynamicImage::Model] the image
    # @!attribute [r] ratio
    #   @return [Float, nil] the aspect ratio to crop to
    # @!attribute [r] sizes
    ...
    # @param options [Hash]
    # @option options [Numeric, Vector2d, String, nil] :ratio The aspect ratio to crop to, as a
    #   number, a vector, or a string like <tt>"16:9"</tt>. Implies cropping. Omit for the image's own.
    ...
    #   fallback image, overriding {DynamicImage.picture_fallback_width}
    # @option options [Symbol, Array<Symbol>] :format The format the candidates are rendered in. A symbol
    #   forces, an array is negotiated. Defaults to WebP, or the image's own format if it is animated.
    # @option options [Boolean] :uncropped Size against the whole image, ignoring any
    #   pre-cropping. Defaults to whether the URL points at the +uncropped+ action.
    #
    ...
    #
    # @return [Array<Hash>] the candidates
    def variants
      @variants ||= widths.map { |width| variant(width) }
    end

    # The +srcset+ attribute for the candidates, or nil if there are none. It belongs on
    # the <tt>source</tt> when {#sources} has one, and on the <tt>img</tt> when it doesn't.
    #
    # @return [String, nil] the srcset
    def srcset
    ...
    #
    # Empty when the candidates are in the same format as the fallback; {#srcset} goes on the
    # <tt>img</tt> instead. That is what happens to an animated image, which keeps its own format.
    #
    # @return [Array<Hash>] the sources
    def sources
      candidates = srcset
    ...
    def variant(width)
      ...
      # {#url_for} fits the size it is given. +size+ has been floored to whole pixels, so it is
      # no longer exactly proportional and fits smaller a second time. Pass the request instead.
