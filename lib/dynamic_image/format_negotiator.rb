...
module DynamicImage
  ...
  #
  # Picks the format to render an image in, given the formats the
  # consumer accepts.
  #
  # The source format is used as-is when it's acceptable. Otherwise the
  # image has to be transcoded, and the candidate that keeps the most of
  # it wins: animation is preserved before transparency, and a
  # constraint no candidate can satisfy is dropped rather than allowed
  # to veto the next one.
  #
  # Selection is keyed on the properties the image actually has, not on
  # what its format is capable of. A photograph uploaded as WebP renders
  # as JPEG, not as a 256 colour GIF.
  #
  # Images stored before +frame_count+ and +alpha+ existed have neither,
  # and an unacceptable source format resolves to JPEG.
  #
  # @example
  ...
  class FormatNegotiator
    ...
    #
    # @param accepted [Array<Symbol, String>] the acceptable format
    #   names, most preferred first. Unrecognized names are ignored.
